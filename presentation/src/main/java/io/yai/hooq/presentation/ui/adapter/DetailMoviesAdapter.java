...
public class DetailMoviesAdapter extends RecyclerView.Adapter<DetailMoviesAdapter.ViewHolder> {

    private Context context;
    private List<MovieModel> data = new ArrayList<>();

    public DetailMoviesAdapter(Context context) {
...
//                    intentToBeCalled.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_SINGLE_TOP);
//                    intentToBeCalled.putExtra(".MOVIE_MODEL", new MovieMapper().serializeModel( data.get(getPosition())) );
//                    context.startActivity(intentToBeCalled);

                    ((MovieDetailActivity)context).setDetail(data.get(getPosition()));
